...
let fatna = new Personnage("Fatna", "Benseddik");
let array = {
    'niveau':"Level 0",
    'niveau1':"Level 1"
};
//document.write("<br>"+ fatna.nom + " " + fatna.prenom + " = "+ array.niveau1);
document.getElementById("priceFilterMin").addEventListener("change", function(e){
    console.log("min = "+this.value + "€");
    document.getElementById("priceValue").innerHtml = this.value + "€";
});
document.getElementById("priceFilterMax").addEventListener("change", function(e){
    console.log("max = "+this.value + "€");
    document.getElementById("priceValue").innerHtml += this.value + "€";
});
document.querySelectorAll("form #priceValue").innerHtml += "€";
...
let product2  = new Product(2,"Eternal Darkness : Sanity's Requiem","Gamecube", 79.90, "Action/Adventure by GhostLight", "media/products/img2.jpg");
...
for (let i=0;i<products.length;i++){
    ...
    <article>
        <aside>
            <figure>
                <img src='`+ products[i].img + `' alt='` + products[i].name + ` `+ products[i].console + `'/>
            </figure>
            <figcaption>
                <h3>`+ products[i].name + `</h3> 
                <p><b>` + products[i].console + `</b></p>
                <p>`+ products[i].price + `€</p>
                <p><i>`+ products[i].desc + `</i></p>
            </figcaption>
        </aside>
